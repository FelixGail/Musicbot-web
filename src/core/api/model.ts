...
type HookReturn<T> = T extends (...args: any[]) => RequestConfig<infer S>? S : any;

export function getHookRequest<S extends (...args: any[]) => RequestConfig<any>>(config: S) {
...
const api = {
	...
	loginUser: (username: string, password: string, includeRefresh?: boolean): RequestConfig<Token> => ({
		url: '/token',
		method: 'GET',
	...
	refreshUser: (token: TokenWithRefresh, includeNewToken?: boolean): RequestConfig<Token> => ({
		url: '/token',
		method: 'GET',
	...
	search: (provider: NamedPlugin, query: string, limit?: number, offset?: number): RequestConfig<Song[]> => ({
	...
	getSuggestions: (suggester: NamedPlugin, max?: number): RequestConfig<Song[]> => ({
	...
	removeSuggestion: (suggester: NamedPlugin, song: Song, provider: NamedPlugin): RequestConfig<void> => ({
		url: `/suggester/${suggester.id}`,
		method: 'DELETE',
		params: {
			songId: song.id,
