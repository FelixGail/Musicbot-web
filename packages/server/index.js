...
program
  ...
  .option(
    "-r --remote <url:port>",
    "address of the musicbot",
    /^https?:\/\/[\S]+:\d+$/i,
    "http://localhost:42945"
  )
  .option(
    "-l --localdomain <host>",
    "host to advertise on .local domain",
    "musicbot"
  )
  .parse(process.argv);

bonjour.publish({
  name: program.localdomain,
  host: `${program.localdomain}.local`,
  type: "http",
  port: program.port
});
...
app.listen(program.port, () => {
  console.log(
    `Server started on port ${program.port}.\n` +
      `Access it via \n  http://${program.localdomain}.local:${
        program.port
